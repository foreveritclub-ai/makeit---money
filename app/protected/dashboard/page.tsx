...
export default function DashboardPage() {
  ...
  const [wallets, setWallets] = useState({ glassBalance: 0, blackBalance: 0 })
    ...
    const loadUserData = async () => {
      try {
        ...
        if (profileError) {
          ...
          if (profileData) {
            setWallets({
              glassBalance: profileData.glass_balance || 0,
              blackBalance: profileData.black_balance || 0,
            })
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    if (
      wallets.glassBalance !== prevWallets.glassBalance ||
